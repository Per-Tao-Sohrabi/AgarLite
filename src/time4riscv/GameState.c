...
void GameState_handle_player_ai_collision(volatile GameState* gs, volatile Player* p, volatile Ai* ai) {
    if (p->area > ai->area) {
        ...
        int new_area_ai = (int) (ai->area * (1.0f - 0.5f)); // Reduce ai area by 50%
        int delta_area_ai = old_area_ai - new_area_ai;

        // Update areas and radii
        ai->area = new_area_ai;
        p->area += delta_area_ai; // Increase p area
        ai->radius = (int) sqrtf(ai->area / 3.14f); // Update AI radius
        p->radius = (int) sqrtf(p->area / 3.14f); // Update player radius
        ...
        // AI eats Player
        int a_ratio = (int) ai->area / p->area;
        
        // ai eats p
        int old_area_p = p->area;
        int new_area_p = (p->area * (int)(1.0f - a_ratio * 0.5f)); // Reduce p2 area
        ...
        p->radius = (int) sqrtf(p->area/3.14f); // Update player radius
        ai->radius = (int) sqrtf(ai->area/3.14f); // Update player radius
        Player_update_velocity(p);
        AI_update_velocity(ai);
        // Update occupied coords dictionary in GameState
        int coord_key_p = (p->x_pos << 16) | p->y_pos;  // Combine x and y into a single key
...
void GameState_handle_player_player_collision(volatile GameState* gs, volatile Player* p1, volatile Player* p2) {
    ...
    int new_area_pj = (int) (pj->area * (1.0f - a_ratio * 0.5f)); // Reduce p2 area
    int delta_area_pj = old_area_pj - new_area_pj;

    // Update areas, radii, and velocities
    pi->area = new_area_pj;
    pi->area += delta_area_pj; // Increase p1 area
    pj->radius = (int) sqrtf(pj->area/3.14f); // Update player radius
    pi->radius =  (int) sqrtf(pi->area/3.14f); // Update player radius
...
void GameState_handle_player_food_collision(volatile GameState* gs, volatile Player* p, volatile Food* f) {
    ...
    print("-------- Player eats");
    int area = p->radius * p->radius * 3.14f; 
    int new_area = area + f->nutrition;
    int new_r = (int) sqrtf(new_area/3.14f);
...
void GameState_handle_ai_ai_collision(volatile GameState* gs, volatile Ai* ai1, volatile Ai* ai2) {
    ...
    int a_ratio = (int) aj->area / ai->area;
    
    // p1 eats p2
    int old_area_aj = aj->area;
    int new_area_aj = (aj->area * (int)(1.0f - a_ratio * 0.5f)); // Reduce p2 area
...
void GameState_handle_ai_food_collision(volatile GameState* gs, volatile Ai* ai, volatile Food* f) {
    ...
    int area = ai->radius * ai->radius * 3.14f; 
    int new_area = area + f-> nutrition;
    int new_r = (int)sqrtf(new_area/3.14f);
